...
export default function Sidebar({ abaAtiva, setAbaAtiva, lidarComLogout, dark, setDark }) {
  ...
  return (
    <aside className="w-64 bg-white dark:bg-zinc-900 border-r border-gray-200/80 dark:border-zinc-800 h-screen sticky top-0 flex flex-col justify-between p-5 select-none hidden md:flex transition-colors duration-200">
      ...
      <div className="border-t border-gray-150 dark:border-zinc-800 pt-4 space-y-1">
        
        {/* BOTÃO DE SOL / LUA LOGO ACIMA DO BOTÃO DE LOGOUT */}
        <button
          type="button"
          onClick={() => setDark(!dark)}
          className="w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs font-bold text-gray-500 dark:text-zinc-400 hover:bg-neutral-50 dark:hover:bg-zinc-800/60 hover:text-gray-900 dark:hover:text-zinc-100 transition-all cursor-pointer"
        >
          {dark ? (
            <>
        ...
        <button
          ...
          className="w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs font-bold text-red-500 hover:bg-red-50 dark:hover:bg-red-950/20 transition-all cursor-pointer"
        >
