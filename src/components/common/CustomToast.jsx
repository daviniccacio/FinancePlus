...
export default function CustomToast({ t, mensagem, tipo = 'success' }) {
  ...
  // Função para fechar manualmente ao clicar no "X"
  const fecharManualmente = () => {
    toast.dismiss(t.id);
    toast.remove(t.id);
  };

  // Quando o tempo limite expira (t.visible = false), remove o elemento do DOM
  useEffect(() => {
    if (!t.visible) {
  ...
  return (
    <div
      className={`max-w-md w-full font-sans flex items-center justify-between p-3.5 rounded-2xl border border-l-4 shadow-2xl backdrop-blur-md transition-all duration-300 ease-in-out bg-white dark:bg-[#161e18] border-gray-200 dark:border-[#273C2C] text-gray-900 dark:text-[#FFE2FE] pointer-events-auto ${
        config.bordaLeft
      } ${
        t.visible
